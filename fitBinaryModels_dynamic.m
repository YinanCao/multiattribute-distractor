...
for s = 1:n_subj
    ...
    % adaptive gain:
    adapGain.outputFull_B{s} = fitFunc_Binary_dyna_AdapGain_FFI(attribute,Y(:,1:2),Y(:,3),minRT,Nfit,...
             distort_flag,[],I0_flag,Tnd_flag,ffi_flag);
    % dual-route (EV)
    EV_DR.outputFull_B{s} = fitFunc_Binary_EV_dyna_DualRoute(attribute,Y(:,1:2),Y(:,3),minRT,Nfit,...
         distort_flag,[],1,I0_flag,Tnd_flag);
    % AU dual-route
    AU_DR.outputFull_B{s} = fitFunc_Binary_AU_dyna_DualRoute(attribute,Y(:,1:2),Y(:,3),minRT,Nfit,...
         distort_flag,[],1,I0_flag,Tnd_flag);
    % EV CI FFI
    ...
    EV_DN.outputFull_B{s} = fitFunc_Binary_EV_dyna_FFI(attribute,Y(:,1:2),Y(:,3),minRT,Nfit,...
         distort_flag,1,[],I0_flag,Tnd_flag,ffi_flag);
    % AU+DN
    AU_DN.outputFull_B{s} = fitFunc_Binary_AUdivN_dyna_FFI(attribute,Y(:,1:2),Y(:,3),minRT,Nfit,...
         distort_flag,1,[],I0_flag,Tnd_flag,ffi_flag);
    ...
    % adap. gain:
    output = fitFunc_Binary_dyna_AdapGain_FFI(attribute(trltrain,:),Y(trltrain,1:2),Y(trltrain,3),minRT,Nfit,...
         distort_flag,[],I0_flag,Tnd_flag,ffi_flag);
    parmEst = output.Xfit; % get best parameters
    adapGain.testLL_B(s,1,fold) = fitFunc_Binary_dyna_AdapGain_FFI(attribute(trltest,:),Y(trltest,1:2),Y(trltest,3),minRT,Nfit,...
         distort_flag,parmEst,I0_flag,Tnd_flag,ffi_flag);
    ...
    % dual-route (AU)
    output = fitFunc_Binary_AU_dyna_DualRoute(attribute(trltrain,:),Y(trltrain,1:2),Y(trltrain,3),minRT,Nfit,...
         distort_flag,[],1,I0_flag,Tnd_flag);
    parmEst = output.Xfit; % get best parameters
    AU_DR.testLL_B(s,1,fold) = fitFunc_Binary_AU_dyna_DualRoute(attribute(trltest,:),Y(trltest,1:2),Y(trltest,3),minRT,Nfit,...
         distort_flag,parmEst,1,I0_flag,Tnd_flag);
    ...
    % AU+DN
    output = fitFunc_Binary_AUdivN_dyna_FFI(attribute(trltrain,:),Y(trltrain,1:2),Y(trltrain,3),minRT,Nfit,...
         distort_flag,1,[],I0_flag,Tnd_flag,ffi_flag);
    parmEst = output.Xfit; % get best parameters
    AU_DN.testLL_B(s,1,fold) = fitFunc_Binary_AUdivN_dyna_FFI(attribute(trltest,:),Y(trltest,1:2),Y(trltest,3),minRT,Nfit,...
         distort_flag,1,parmEst,I0_flag,Tnd_flag,ffi_flag);
...
if toSave
save(['Binary_models_',time_str,'.mat'],...
    'datafile',...
    'rankSI',...
    'adapGain',...
    'EV_DR',...
